...
namespace EAP.ModelFirst.Controls
{
    public class DiagramNavigator : Control
    {
        ...
        {
            Invalidate(); //TODO: This could perhaps be optimized, so as not to invite as many ...
        }
        ...
        private void DrawDocument(Graphics g)
        {
            if (DocumentVisualizer != null && DocumentVisualizer.HasDocument)
            {
                ...
                Rectangle borders = new Rectangle(0, 0, (int)(DocumentVisualizer.DocumentSize.Width * zoom), (int)(DocumentVisualizer.DocumentSize.Height * zoom));
                g.FillRectangle(brush, borders);
                ...
                {
                    //TODO it will cause an overflow exception when drawing GUI+ 
                    //and i don't know the reason and how to fix, so eat the error here
                }
