...
namespace EAP.ModelFirst.Controls.Editors.DiagramEditor.ClassDiagram.Shapes
{
    public abstract class Shape : DiagramElement
    {
        ...
        protected Shape(IEntity entity)
        {
            ...
            entity.Modified += new EventHandler(entity_Modified);
        }

        void entity_Modified(object sender, EventArgs e)
        {
        ...
        {
            return ShapeKryptonContextMenu.Default.GetMenuItems(diagram);
        }
        ...
        {
            XmlElement locationNode = node["Location"];
            if (locationNode != null)
            {
                this.Location = new Point(locationNode.GetAttributeValue("left", 0),
                    locationNode.GetAttributeValue("top", 0));
            }

            XmlElement sizeNode = node["Size"];
            if (sizeNode != null)
            {
                this.Size = new Size(sizeNode.GetAttributeValue("width", 0),
                    sizeNode.GetAttributeValue("height", 0));
            }
        }
