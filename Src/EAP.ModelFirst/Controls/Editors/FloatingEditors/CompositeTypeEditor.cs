...
namespace EAP.ModelFirst.Controls.Editors.FloatingEditors
{
	...
	public partial class CompositeTypeEditor : EditorWindow
	{
		CompositeTypeShape shape = null;
		bool needValidation = false;

        class CompositeTypeEditorHandler : TypeEditorHandler
        {
            CompositeTypeEditor editor { get { return Window as CompositeTypeEditor; } }
        ...
        public CompositeTypeEditor()
            : base(new CompositeTypeEditorHandler())
        {
            InitializeComponent();
		...
		{
			bool valid = false;
            switch (FloatingEditorHandler.NewMemberType)
			{
			...
			if (!valid)
			{
                FloatingEditorHandler.NewMemberType = MemberType.Method;
				toolNewMember.Image = Properties.Resources.NewMethod;
				toolNewMember.Text = Strings.NewMethod;
			}
		...
		{
            AddNewMember(FloatingEditorHandler.NewMemberType);
		}
		...
		private void AddNewMember(MemberType type)
		{
            ...
            FloatingEditorHandler.NewMemberType = type;
			switch (type)
			{
				case MemberType.Field:
					if (shape.CompositeType.SupportsFields)
