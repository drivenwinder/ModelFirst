...
{
	internal sealed class UmlField : Field
	{
		...
		const string FieldPattern =
			@"^\s*" + UnifiedModelingLanguage.AccessPattern + ModifiersPattern +
			@"(?<type>" + UnifiedModelingLanguage.GenericTypePattern2 + @")\s+" +
			@"(?<name>" + UnifiedModelingLanguage.NamePattern + @")" +
			@"(\s*=\s*" + InitValuePattern + ")?" + UnifiedModelingLanguage.DeclarationEnding;
