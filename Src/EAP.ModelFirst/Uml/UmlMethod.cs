...
{
	internal sealed class UmlMethod : Method
	{
		...
		const string ConvOperatorPattern = @"(?<convop>implicit|explicit)" +
			@"\s+operator\s+(?<operator>" + UnifiedModelingLanguage.GenericTypePattern2 + ")";
		...
		const string MethodPattern =
			...
			@"(?<name>" + OperatorPattern + "|" + UnifiedModelingLanguage.GenericOperationNamePattern + "))" +
			@"\s*\((?<args>.*)\)" + UnifiedModelingLanguage.DeclarationEnding;

		// [explicit | implicit] operator <operator> | <name>
		const string MethodNamePattern =
			@"^\s*(?<name>((?<convop>implicit\s+|explicit\s+)?operator" + 
			@"(?<operator>(?(convop)\s+" + UnifiedModelingLanguage.GenericTypePattern2 + @"|\s*" + 
			OverloadableOperators + @"))|" + UnifiedModelingLanguage.GenericOperationNamePattern + @"))\s*$";
		...
		/// </exception>
		internal UmlMethod(CompositeType parent) : this("NewMethod", parent)
		{
		}
		...
		/// <paramref name="parent"/> is null.
		/// </exception>
		internal UmlMethod(string name, CompositeType parent) : base(name, parent)
		{
		}
		...
		{
			UmlMethod method = new UmlMethod(newParent);
